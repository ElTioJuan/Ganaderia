...
USE ganaderia;
-- DROP TABLE  IF EXISTS user, breed;
-- Platform
CREATE TABLE user (
...
CREATE TABLE cow(
    ...
    cow_desc TEXT,
    alive BOOLEAN,
    heat BOOLEAN, -- Vaca en celo(heat)
    gender VARCHAR(10) DEFAULT 'MACHO',
    CHECK (
        gender = 'MACHO'
        OR gender = 'HEMBRA'
    ),
    cow_shoes BOOLEAN,  -- Herradura
    dehorned BOOLEAN,  -- descornado
    periodic_weight DOUBLE, -- peso_Periodico
    periodic_height DOUBLE, -- altura_periodica
    clinic_history TEXT, 
    medic_history TEXT,
...
CREATE TABLE cowbirth(
    ...
    id_cow INT(12),
    id_father INT(12),                 
    id_mother INT(12),                -- ESTIMADO DE EMBARAZO DE VACAS ES 9 MESES Y 10 DIAS
    monta_date DATE NOT NULL,
    
    birth_date DATE, -- abarca rambién la fecha de parida
